...
const total = items.reduce((a, b) => a + b.amount, 0)  // number 추론`,
            ...
            {
                n: '④',
                title: '배포를 기다리지 않게 · 중복을 걷어낸 파이프라인',
                rows: [
                    {
                        label: '해결',
                        content:
                            '빌드마다 의존성을 새로 받고 단계마다 빌드를 다시 돌리던 CI/CD를, 캐시와 산출물 재사용으로 정리. 빌드·배포를 10분→2분 30초로 줄여, 이 배포를 기다리던 6명의 대기시간까지 함께 덜어냄',
                    },
                ],
                code: {
                    tag: '구조 재현 · 회사 코드 아님',
                    panes: [
                        {
                            label: '단계마다 반복되던 낭비',
                            note: '파이프라인이 install → build → CDN 배포 → web 배포로 흘렀는데, 두 군데서 같은 일을 반복하고 있었어요. 하나는 pnpm store 캐시가 동작하지 않아 매 실행마다 의존성을 처음부터 새로 받는 것, 다른 하나는 뒤 단계가 앞 단계의 빌드 산출물을 물려받지 못해 배포 단계에서 빌드를 다시 도는 것. 트래픽 구조(①)만큼 눈에 띄진 않지만, 하루에도 여러 번 도는 배포라 팀 전체 대기시간으로 쌓였어요.',
                            code: `[ before ]

install ─── 의존성 새로 다운로드 (캐시 미동작)
   ▼
build ───── 빌드
   ▼
CDN 배포 ── 여기서 또 빌드 (산출물 안 물려받음)
   ▼
web 배포 ── 여기서 또 빌드
   ▼
10분 · 매번 처음부터`,
                        },
                        {
                            label: '캐시와 산출물을 물려주기',
                            highlight: true,
                            note: '두 낭비를 각각 끊었어요. pnpm store를 캐시해 의존성은 바뀔 때만 받게 하고, 앞 단계 빌드 산출물(artifact)을 뒤 단계로 물려줘 배포 단계가 빌드를 다시 돌지 않게 했어요. "한 번 한 일을 다시 하지 않는다"를 파이프라인에 적용한 거예요. 빌드·배포는 한 번에 7분 30초를 줄였지만, 진짜 효과는 그 뒤에 있었어요 — 이 배포를 기다리던 6명이 매번 함께 멈춰 있었거든요. 1인당 월 평균 12.5시간이 여기서 나왔어요. 이후로 개선 과제를 고를 때, 효과의 크기뿐 아니라 얼마나 자주·몇 사람이 겪는 일인지를 함께 따지게 됐어요.',
                            code: `[ after ]

install ─── pnpm store 캐시 (바뀔 때만 받음)
   ▼
build ───── 빌드 → artifact 저장
   ▼
CDN 배포 ── artifact 재사용 (빌드 X)
   ▼
web 배포 ── artifact 재사용 (빌드 X)
   ▼
2분 30초 · 중복 제거`,
                        },
                    ],
        ...
        results: [
            ...
            { v: '1,564만', k: '신청 전부 처리, 600만 MAU' },
            { v: '10분 → 2.5분', k: '빌드·배포 (팀 대기 12.5h↓)' },
        ],
