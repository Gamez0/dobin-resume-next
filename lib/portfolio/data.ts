...
export const PORTFOLIO_CASES: IPortfolioCase[] = [
    // (배경 + ① 까지 반영 / ②③·결과·배움은 추후 — draft 는 풀되 본문 일부만 채운 상태)
    {
        ...
        background: [
            `상생페이백은 전국민이 대상이라, 오픈과 동시에 수천만이 한 번에 몰리는 서비스였어요. 그런데 인프라는 NHN 계약으로 고정 — 서버를 마음대로 늘릴 수도, SSR을 쓸 수도 없었어요. 게다가 정책이 3일마다 바뀌었고, 그 변경을 여러 팀원이 동시에 쳐내야 했어요. 그래서 우선순위가 분명했어요. 체감 속도를 끌어올리는 교과서적 최적화보다, 사용자가 한꺼번에 쏟아져도 전부에게 실패 없이 deliver하는 것. 그래서 무겁게 다듬기보다 가볍게 만드는 데 초점을 뒀어요. 대규모는 트래픽만이 아니라, 변경과 협업의 규모이기도 했어요.`,
        ],
        decisions: [
            {
                n: '①',
